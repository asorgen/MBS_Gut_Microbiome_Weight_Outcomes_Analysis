...
prevModule <- paste0("PathMetaMerge")
...
for (i in 1:ncol(myT)){
  ...
  if (mean(path>0, na.rm = TRUE)>0.1){
    ...
    df<-data.frame(path,PatientID, Timepoint)
    df <- df[df$Timepoint %in% included,]
    df <- na.omit(df)
    df$Timepoint <- as.factor(df$Timepoint)
    
    mlm <- lme(path ~ Timepoint, method="REML", random=~1 | PatientID, data=df)
    smry <- summary(mlm)
    fit<-anova(mlm)
    pValue.MLM[index]<-fit$`p-value`[2]
    
    lm <- lm(df$path ~ df$Timepoint)
    smry <- summary(lm)
    fit<-anova(lm)
...
path.df<-data.frame(pathName, pValue.MLM, pValue.LM, p_BLv1M)
path.df$Adj_pValue.MLM <- p.adjust(path.df$pValue.MLM,method = "BH")
path.df$Adj_pValue.LM <- p.adjust(path.df$pValue.LM,method = "BH")
path.df$Adj_p_BLv1M<- p.adjust(path.df$p_BLv1M,method = "BH")

if (length(included) > 2) {
  path.df$p_BLv6M <- p_BLv6M
  path.df$Adj_p_BLv6M <- p.adjust(path.df$p_BLv6M,method = "BH")
}

if (length(included) > 3) {
  path.df$p_BLv12M <- p_BLv12M
  path.df$Adj_p_BLv12M <- p.adjust(path.df$p_BLv12M,method = "BH")
}

if (length(included) > 4) {
  path.df$p_BLv18M <- p_BLv18M
  path.df$Adj_p_BLv18M <- p.adjust(path.df$p_BLv18M,method = "BH")
}

if (length(included) > 5) {
  path.df$p_BLv24M <- p_BLv24M
  path.df$Adj_p_BLv24M <- p.adjust(path.df$p_BLv24M,method = "BH")
}

pVal.FileName <- paste0("Pathway_by_Timepoint_mixedLM_and_LM_BLto", included[length(included)], "M_", classifier, ".tsv")
file.path <- paste0(outputDir, pVal.FileName)
write.table(path.df, file.path,sep="\t",quote = FALSE, row.names = FALSE)




file.path <- paste0(outputDir, "Pathway_by_Timepoint_mixedLM_and_LM_pValHistogram_BLto", included[length(included)], "M_", classifier, ".pdf")

pdf(file.path)

pValues <- path.df[,which(colnames(path.df) == "pValue.MLM")]
n=length(pValues)
title.lab <- paste0(classifier, "\nMixed LM (path ~ Timepoint) p values (n = ", n, ")")
hist(pValues, xlab = "p value bins", main = title.lab, breaks = seq(0, 1, 0.05))

pValues <- path.df[,which(colnames(path.df) == "pValue.LM")]
n=length(pValues)
title.lab <- paste0(classifier, "\nLM (path ~ Timepoint) p values (n = ", n, ")")
hist(pValues, xlab = "p value bins", main = title.lab, breaks = seq(0, 1, 0.05))

pValues <- path.df[,which(colnames(path.df) == "p_BLv1M")]
n=length(pValues)
title.lab <- paste0(classifier, "\nLinear model BL v. 1 month p values (n = ", n, ")")
hist(pValues, xlab = "p value bins", main = title.lab, breaks = seq(0, 1, 0.05))

if (length(included) > 2) {
  pValues <- path.df[,which(colnames(path.df) == "p_BLv6M")]
  n=length(pValues)
...
if (length(included) > 3) {
  pValues <- path.df[,which(colnames(path.df) == "p_BLv12M")]
  n=length(pValues)
...
if (length(included) > 4) {
  pValues <- path.df[,which(colnames(path.df) == "p_BLv18M")]
  n=length(pValues)
...
if (length(included) > 5) {
  pValues <- path.df[,which(colnames(path.df) == "p_BLv24M")]
  n=length(pValues)
...
file.path <- paste0(outputDir, pVal.FileName)
pVal.df <- read.delim(file.path, sep="\t",header = TRUE, row.names = 1,check.names = FALSE)
...
for (row in 1:nrow(ordered)) {
  ...
  pathName <- rownames(ordered[row,])
  
  pathCol <- logCounts2[,which(colnames(logCounts2) == pathName)]
  month <- logCounts2$time
  df <- data.frame(pathCol, month)
  df <- na.omit(df)
  
  stat.test <- df %>%
    t_test(pathCol ~ month, ref.group = "0") %>%
  ...
  plot <- plot +
    labs(title = paste0(pathName, " (", ordered$pFinal[row], ")"))
...
# Output plot
file.path <- paste0(outputDir, "Pathway_by_Timepoint_MLM_BLto", included[length(included)], "M_Boxplots_", classifier, ".pdf")
...
for (i in 1:ncol(myT)){
  ...
  if (mean(path>0, na.rm = TRUE)>0.1){
    ...
    message(">>", i, ".) ", colnames(myT)[i])
    df2<-data.frame(path, surgery, month)
    ...
    plot <- ggboxplot(
      df2, x = "surgery", y = "path", color = "month",
      # fill = "#2EDF52",
      # facet.by = "month",
      scales = "free", add = "jitter"
    )
...
write.table(t.testResults, paste0(outputDir, "Pathway_by_Surgery_ttest_BLto", included[length(included)], "M_Results_", classifier, ".tsv"),sep="\t",row.names = FALSE,quote = FALSE)

# Output plot
file.path <- paste0(outputDir, "Pathway_by_Surgery_ttest_BLto", included[length(included)], "M_Boxplots_", classifier, ".pdf")
...
for (division in divisions) {
  message(paste0("\n***** Starting multivariate (", division, ") *****\n"))
  ...
  dFrame<-data.frame(pathName, pVal_Timepoint, pVal_Surgery, pVal_Weight_kg)
  ...
  write.table(dFrame, paste0(outputDir, "Pathway_by_Timepoint_Surgery_Weight_kg__mixedLM_BLto", included[length(included)], "M_", classifier, "_RESULTS.tsv"),sep="\t",row.names = FALSE,quote = FALSE)
...
##### Univariate path change linear model #####

message(paste0("\n***** Starting change univariate analysis *****\n"))
...
for (i in 1:ncol(myT)){
  ...
  if (mean(path>0, na.rm = TRUE)>0.1){
    ...
    # baseline abundance
    BL_path <- df[bl, "path"]
    names(BL_path) <- df[bl, "PatientID"]
    
    # Add path info to each row
    df$BL_path <- BL_path[df$PatientID]
    
    # Remove baseline rows (won't show any change)
    df <- df[df$Timepoint != 0,]
    df$pathChange <- df$path - df$BL_path
    df$pathPercentChange <- ( df$pathChange / df$BL_path ) * 100
    
    df$Timepoint <- as.factor(df$Timepoint)
    
    df <- df[!(is.na(df$pathChange)),]
    
    mlm <- lme(pathChange ~ Timepoint, method="REML", random=~1 | PatientID, data=df)
    smry <- summary(mlm)
    fit<-anova(mlm)
    pValue.MLM[index]<-fit$`p-value`[2]
    
    lm <- lm(df$pathChange ~ df$Timepoint)
    smry <- summary(lm)
    fit<-anova(lm)
    pValue.LM[index]<-fit$`Pr(>F)`[1]
    # p_BLv1M[index] <- smry$coefficients[2,4]
...
# path.df<-data.frame(pathName, pValue.MLM, pValue.LM, p_BLv1M)
path.df<-data.frame(pathName, pValue.MLM, pValue.LM)
path.df$Adj_pValue.MLM <- p.adjust(path.df$pValue.MLM,method = "BH")
path.df$Adj_pValue.LM <- p.adjust(path.df$pValue.LM,method = "BH")
# path.df$Adj_p_BLv1M<- p.adjust(path.df$p_BLv1M,method = "BH")
# path.df$BLv1M_sig <- sigStars(path.df$Adj_pValue.MLM)

# if (length(included) > 2) {
#   path.df$p_BLv6M <- p_BLv6M
#   path.df$Adj_p_BLv6M <- p.adjust(path.df$p_BLv6M,method = "BH")
#   path.df$BLv6M_sig <- sigStars(path.df$Adj_pValue.MLM)
# }
# 
# if (length(included) > 3) {
#   path.df$p_BLv12M <- p_BLv12M
#   path.df$Adj_p_BLv12M <- p.adjust(path.df$p_BLv12M,method = "BH")
#   path.df$BLv12M_sig <- sigStars(path.df$Adj_pValue.MLM)
# }
# 
# if (length(included) > 4) {
#   path.df$p_BLv18M <- p_BLv18M
#   path.df$Adj_p_BLv18M <- p.adjust(path.df$p_BLv18M,method = "BH")
#   path.df$BLv18M_sig <- sigStars(path.df$Adj_pValue.MLM)
# }
# 
# if (length(included) > 5) {
#   path.df$p_BLv24M <- p_BLv24M
#   path.df$Adj_p_BLv24M <- p.adjust(path.df$p_BLv24M,method = "BH")
#   path.df$BLv24M_sig <- sigStars(path.df$Adj_pValue.MLM)
# }

file.path <- paste0(outputDir, "Change_by_Timepoint_mixedLM_and_LM_Results_BLto", included[length(included)], "M_", classifier, ".tsv")
write.table(path.df, file.path,sep="\t",quote = FALSE, row.names = FALSE)




file.path <- paste0(outputDir, "Change_by_Timepoint_MLM_and_LM_pValHistogram_BLto", included[length(included)], "M_", classifier, ".pdf")
...
for (division in divisions) {
  message(paste0("\n***** Starting ", division, " top v bottom *****\n"))
  outputWLdivisions <- paste0(outputDir, division, "/")
  dir.create(outputWLdivisions, showWarnings = FALSE)
  ...
  for (i in 1:ncol(myT)){
    ...
    if (mean(path>0, na.rm = TRUE)>0.1){
      ...
      df2<-data.frame(path, WLGroup, month)
      ...
      plot <- ggboxplot(
        df2, x = "WLGroup", y = "path", color = "month",
        # fill = "#2EDF52",
        # facet.by = "month",
        scales = "free", add = "jitter"
      )
      ...
      plot <- plot +
        labs(x = paste0("Weight Loss ", division, "s"), y = "Pathway Abundance")
  ...
  write.table(t.testResults, paste0(outputWLdivisions, "Pathway_by_Top_Bottom_", division, "_ttest_BLto", included[length(included)], "M_Results_", classifier, ".tsv"),sep="\t",row.names = FALSE,quote = FALSE)
  
  # Output plot
  file.path <- paste0(outputWLdivisions, "Pathway_by_Top_Bottom_", division, "_ttest_BLto", included[length(included)], "M_Boxplots_", classifier, ".pdf")
  pdf(file.path, width = 7, height = 5)
  for (i in 1:length(plotList)) {
    grid.arrange(plotList[[i]], ncol = 1, nrow = 1)
  }
