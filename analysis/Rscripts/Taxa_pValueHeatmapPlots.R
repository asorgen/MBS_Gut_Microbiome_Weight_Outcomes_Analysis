...
moduleRoot <- "Taxa_pValueHeatmapPlots"
...
for (level in levels) {
  ...
  for (month in months) {
    ...
    prevModule <- paste0(classifier, "_Taxa_over_time_BLto", month, "M")
    ...
    file.path <- paste0(inputLevel, level, "_by_Timepoint_", division, "_LM_pValues_BLto", month, "M_", classifier, ".tsv")
  ...
  message("\n>>>>> Outputting plot")
  # Output plot
  file.path <- paste0(outputLevel, level, "_Heatmap.pdf")
