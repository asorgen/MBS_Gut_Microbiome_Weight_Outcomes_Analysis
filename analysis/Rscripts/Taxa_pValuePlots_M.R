...
#Date: 2023 Jan 31
#Description: Generate p value plots for taxa changes over time for 24M weight classifications.
...
if (args[1] == "BLJ") {
  ...
  if (args[2] == "MetaPhlAn2" | args[2] == "Kraken2") {
    module <- gsub("Taxa", args[2], moduleRoot)
  } 
  ...
  if (exists("end_month") == TRUE) {
    ...
    module <- paste0(mod1, "_", end_month, "M", mod2, "_BLto24M")
    rm(mod1, mod2)
  }
...
prevModule <- paste0(classifier, "_Taxa_over_time_", endM, "M_Weight_Class_BLto", months[length(months)], "M")
...
for (level in levels) {
  ...
  file.path <- paste0(inputLevel, level, "_by_Timepoint_", division, "_LM_pValues_BLto24M_", classifier, ".tsv")
  ...
  for ( timeFrame in timeFrames[1:length(unique(qFrame$LengthTime))] ) {
    ...
    title.lab <- paste0(classifier, " ", endM, "M ", division, " ", str_to_title(level),  " level taxonomic changes over time (BL to ", endPoint, ")"); title.lab
    ...
    # Print the plot
    file.path <- paste0(outputLevel, level, paste0("_by_Timepoint_", endM, "M_", division, "_LM_pValue_boxplot_", timeFrame, "_", classifier, ".pdf"))
  ...
  for ( timeFrame in timeFrames[1:length(unique(qFrame$LengthTime))] ) {
    ...
    plot <- plot + annotate("segment", x=0.05, y=0.25, xend=0.05, yend=2,
                            col="black", arrow=arrow(length=unit(0.5, "cm"))); plot
    plot <- plot + annotate("text", x=0.1, y=1.6, hjust = 0, label = "Increases in abundance"); plot
    ...
    plot <- plot + annotate("text", x=0.1, y=-1.6, hjust = 0, label = "Decreases in abundance"); plot
    
    # Print the plot
    file.path <- paste0(outputLevel, level, paste0("_by_Timepoint_", endM, "M_", division, "_LM_pValue_scatter_", timeFrame, "_", classifier, ".pdf"))
  ...
  pVal.FileName <- paste0(level, "_by_Timepoint_FisherResults.tsv")
  file.path <- paste0(outputLevel, pVal.FileName)
