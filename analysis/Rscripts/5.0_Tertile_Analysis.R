  ...
  for ( t in 2:length(months) ) {
    ...
    for( j in 1:ng ) {
      ...
      tryCatch({
        ...
        fit<-anova(mlm)
        groupPValues3[qIndex] <- fit$"p-value"[3]
      }, error = function(e){ groupPValues3[qIndex] <<- NA })

      tryCatch({
        mlm <- lme( bugCol ~ Month, method = "REML", random = ~1 | PatientID, data = lm.df )
        fit<-anova(mlm)
        groupPValues4[qIndex] <- fit$"p-value"[2]
      }, error = function(e){ groupPValues4[qIndex] <<- NA })
...
for ( timeFrame in timeFrames[1:length(unique(qFrame$LengthTime))] ) {
  ...
  f.pVal <- tryCatch({
    ...
    paste0("p = ", format(f.test$p.value, digits = 3))
