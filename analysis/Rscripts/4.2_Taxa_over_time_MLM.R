...
moduleRoot <- "4.2_Taxa_over_time_MLM"
...
for (level in levels) {
  ...
  for (t in 1:length(months)) {
    if (t != length(months)) {
      ...
      for (t1 in otherMonths) {
        ...
        if (level == "genus") {
          toRemove <- which(colnames(myT) %like% "ales_noname")
          if (length(toRemove) > 0) myT <- myT[, -toRemove]
          toRemove <- which(colnames(myT) %like% "Candidatus_Saccharibacteria")
          if (length(toRemove) > 0) myT <- myT[, -toRemove]
          toRemove <- which(colnames(myT) %in% c("Bacteroidetes_noname"))
          if (length(toRemove) > 0) myT <- myT[, -toRemove]
        }
  ...
  for (t in 1:length(months)) {
    if (t != length(months)) {
      ...
      if (level == "genus") {
        toRemove <- which(colnames(myT) %like% "_noname")
        if (length(toRemove) > 0) myT <- myT[, -toRemove]
        toRemove <- which(colnames(myT) %like% "_unclassified")
        if (length(toRemove) > 0) myT <- myT[, -toRemove]
      }
      PatientID <- log.df$PatientID
      Timepoint <- log.df$time
